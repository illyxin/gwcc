...
--============================================================
local MOTION = {
    speed = 1.0, reduce = false, blur = true, dim = true,
    shimmer = true, idleFloat = true, edgeSnap = true,
}
...
--============================================================
local C = {
    Panel   = Color3.fromRGB(13, 13, 18),  PanelLt = Color3.fromRGB(17, 17, 24),
    HdrTop  = Color3.fromRGB(19, 19, 26),  NavCol  = Color3.fromRGB(15, 15, 20),
    NavAct  = Color3.fromRGB(26, 26, 36),  NavIna  = Color3.fromRGB(20, 20, 26),
    NavHov  = Color3.fromRGB(22, 22, 30),  TxtPri  = Color3.fromRGB(228, 228, 232),
    TxtMut  = Color3.fromRGB(106, 106, 120), TxtBrt = Color3.fromRGB(145, 145, 160),
    TxtHint = Color3.fromRGB(74, 74, 85),   Accent  = Color3.fromRGB(90, 90, 122),
    AccentH = Color3.fromRGB(140, 140, 190), BarBg  = Color3.fromRGB(24, 24, 32),
    BarA    = Color3.fromRGB(74, 74, 106),  BarB   = Color3.fromRGB(106, 106, 138),
    BarDone = Color3.fromRGB(130, 130, 175), ScrClr = Color3.fromRGB(42, 42, 53),
    MinA    = Color3.fromRGB(26, 26, 36),   MinB   = Color3.fromRGB(20, 20, 26),
    MinP    = Color3.fromRGB(30, 30, 40),   StrkClr = Color3.fromRGB(26, 26, 34),
    StrkAct = Color3.fromRGB(58, 58, 82),   HLne   = Color3.fromRGB(23, 23, 31),
...
local function ti(dur, style, dir)
    return TweenInfo.new(
        ...
        style or ES.Quint, dir or ED.Out)
end
...
function Motion.to(obj, group, info, props)
    ...
    if reg[group] then reg[group]:Cancel() end
    local t = TweenService:Create(obj, info, props)
    reg[group] = t; t:Play(); return t
end
...
function Motion.spring(scaleObj, target, opts)
    ...
    opts = opts or {}
    if MOTION.reduce then Motion.to(scaleObj, "scale", E.snap(), { Scale = target }); return end
    ...
    if st then
        ...
        springPool[scaleObj] = { target = target, vel = opts.impulse or 0,
            stiffness = opts.stiffness or 260, damping = opts.damping or 22 }
    end
    if not springConn then
        springConn = RunService.RenderStepped:Connect(function(dt)
            dt = math.min(dt, 1/30); local alive = 0
            for obj, st in pairs(springPool) do
                if not obj.Parent then springPool[obj] = nil
                else
                    local x = obj.Scale
                    ...
                    if math.abs(st.target - obj.Scale) < 0.0015 and math.abs(st.vel) < 0.02 then
                        obj.Scale = st.target; springPool[obj] = nil
                    else alive = alive + 1 end
                end
            end
            if alive == 0 then springConn:Disconnect(); springConn = nil end
        end)
    end
end
...
function Motion.stagger(list, step, startAt, fn)
    if MOTION.reduce then for i, v in ipairs(list) do fn(v, i) end; return end
...
local function borderStroke(parent, color, thick, transp)
    return new("UIStroke", {
        Color = color or C.StrkClr, Thickness = thick or 1,
...
local function snapshot(root)
    local snap, all = {}, root:GetDescendants()
    table.insert(all, 1, root)
    for _, o in ipairs(all) do
        if o:IsA("GuiObject") then snap[#snap+1] = { o, "BackgroundTransparency", o.BackgroundTransparency } end
...
local function fadeSnapshot(snap, info, toHidden)
    for _, e in ipairs(snap) do
        if e[1] and e[1].Parent then
            Motion.to(e[1], "fade_"..e[2], info, { [e[2]] = toHidden and 1 or e[3] })
        end
    end
end
...
local gui = new("ScreenGui", {
    Name = "gwcc_UI", ResetOnSpawn = false, IgnoreGuiInset = true,
...
local dim = new("Frame", {
    ...
    Active = false, Size = UDim2.fromScale(1, 1), ZIndex = 5,
}, gui)

local blur
if MOTION.blur then
    pcall(function() blur = new("BlurEffect", { Name = "gwcc_Blur", Size = 0, Enabled = true }, Lighting) end)
end

local function setBackdrop(on)
    if MOTION.dim then Motion.to(dim, "bg", E.smooth(), { BackgroundTransparency = on and 0.45 or 1 }) end
    if blur then Motion.to(blur, "blur", E.smooth(), { Size = on and 14 or 0 }) end
end
...
local welcome = new("Frame", {
    Name = "Welcome", AnchorPoint = Vector2.new(0.5, 0.5),
    Position = welcomeTargetPos + UDim2.fromOffset(0, 34),
    Size = UDim2.fromOffset(WW, 170), BackgroundColor3 = C.Panel,
    BorderSizePixel = 0, Rotation = 1.5, ZIndex = 50,
}, gui)
corner(welcome, 10)
...
local wTitle = new("TextLabel", {
    Name = "Title", BackgroundTransparency = 1, Text = "Welcome to gw.cc",
    TextColor3 = C.TxtPri, TextTransparency = 1,
    TextXAlignment = Enum.TextXAlignment.Center, Font = FONT, TextSize = 18,
    Size = UDim2.new(1, 0, 0, 24), Position = UDim2.new(0, 0, 0, 28), ZIndex = 1,
}, welcome)

local wCredit = new("TextLabel", {
    Name = "Credit", BackgroundTransparency = 1, Text = "by illyxin",
    TextColor3 = C.TxtMut, TextTransparency = 1,
    TextXAlignment = Enum.TextXAlignment.Center, Font = FONT, TextSize = 12,
...
new("UIGradient", { Transparency = NumberSequence.new({
    NumberSequenceKeypoint.new(0, 1), NumberSequenceKeypoint.new(0.5, 0.55),
    NumberSequenceKeypoint.new(1, 1),
}) }, shimmer)

local wPct = new("TextLabel", {
    Name = "Percent", BackgroundTransparency = 1, Text = "Loading... 0%",
    TextColor3 = C.TxtBrt, TextTransparency = 1,
    TextXAlignment = Enum.TextXAlignment.Center, Font = FONT, TextSize = 11,
...
new("Frame", { Name = "HFoot", BackgroundColor3 = C.Panel, BorderSizePixel = 0,
    AnchorPoint = Vector2.new(0, 1), Position = UDim2.new(0, 0, 1, 0),
    Size = UDim2.new(1, 0, 0, 8), ZIndex = 1 }, header)

new("Frame", { Name = "HLine", BackgroundColor3 = C.HLne, BorderSizePixel = 0,
    AnchorPoint = Vector2.new(0, 1), Position = UDim2.new(0, 0, 1, 0),
    Size = UDim2.new(1, 0, 0, 1), ZIndex = 2 }, header)
...
local brand = new("TextLabel", {
    ...
    Font = FONT, TextSize = 15, AnchorPoint = Vector2.new(0, 0.5),
...
local hint = new("TextLabel", {
    ...
    Font = FONT, TextSize = 10, AnchorPoint = Vector2.new(1, 0.5),
...
new("Frame", { Name = "NavTopFill", BackgroundColor3 = C.NavCol, BorderSizePixel = 0,
    Size = UDim2.new(1, 0, 0, 8), ZIndex = 1 }, nav)
...
for i, id in ipairs(TABS) do
    local btn = new("TextButton", {
        ...
        BorderSizePixel = 0, Text = id, TextColor3 = C.TxtMut, Font = FONT,
...
new("Frame", { Name = "NavSep", BackgroundColor3 = C.HLne, BorderSizePixel = 0,
    AnchorPoint = Vector2.new(1, 0), Position = UDim2.new(1, 0, 0, 0),
    Size = UDim2.new(0, 1, 1, 0), ZIndex = 2 }, nav)

local function startGlow(id)
    if navGlow[id] then return end
    local acc = navAccs[id]
    if not acc or MOTION.reduce then return end
    navGlow[id] = TweenService:Create(acc,
        TweenInfo.new(1.7, ES.Sine, ED.InOut, -1, true), { BackgroundColor3 = C.AccentH })
    navGlow[id]:Play()
end

local function stopGlow(id)
    if navGlow[id] then navGlow[id]:Cancel(); navGlow[id] = nil
        if navAccs[id] then navAccs[id].BackgroundColor3 = C.Accent end end
end
...
local function styleNav(id, active)
    ...
    if active then
        ...
        local grow = Motion.to(acc, "acc", ti(0.3, ES.Quint, ED.Out), {
            BackgroundTransparency = 0, Size = UDim2.new(0, 3, 1, -16),
        })
        Motion.spring(scl, 1, { impulse = 0.85, stiffness = 300, damping = 17 })
        if grow and not MOTION.reduce then grow.Completed:Once(function() startGlow(id) end) end
    else
        Motion.to(acc, "acc", E.exit(), {
            BackgroundTransparency = 1, Size = UDim2.new(0, 3, 0, 0),
        })
        Motion.spring(scl, 1)
    end
end
...
local function mkContentLabel()
    return new("TextLabel", {
        BackgroundTransparency = 1, Text = "", TextColor3 = C.TxtPri,
        TextTransparency = 1, TextXAlignment = Enum.TextXAlignment.Center,
        TextYAlignment = Enum.TextYAlignment.Center, Font = FONT, TextSize = 18,
        AnchorPoint = Vector2.new(0.5, 0.5), Position = UDim2.fromScale(0.5, 0.5),
        Size = UDim2.new(1, -20, 0, 30), ZIndex = 2,
    }, content)
end
...
local function crossfadeContent(text, size)
    ...
    incoming.Text = text; incoming.TextSize = size or 15
    incoming.TextTransparency = 1; incoming.Position = UDim2.new(0.5, 0, 0.5, 14)
    incoming.ZIndex = 3; outgoing.ZIndex = 2
...
corner(minBtn, 12); dropShadow(minBtn, 34, 0.6)
...
local icon1 = new("Frame", { Name = "Icon1", BackgroundColor3 = C.TxtMut,
    BorderSizePixel = 0, AnchorPoint = Vector2.new(0.5, 0.5),
    Position = UDim2.new(0.5, 0, 0.5, -4), Size = UDim2.fromOffset(18, 2), ZIndex = 1 }, minBtn)
corner(icon1, 1)
local icon2 = new("Frame", { Name = "Icon2", BackgroundColor3 = C.TxtMut,
    BorderSizePixel = 0, AnchorPoint = Vector2.new(0.5, 0.5),
    Position = UDim2.new(0.5, 0, 0.5, 4), Size = UDim2.fromOffset(12, 2), ZIndex = 1 }, minBtn)
corner(icon2, 1)

local function morphIcon(open)
    local info = ti(0.34, ES.Back, ED.Out)
    if open then
        Motion.to(icon1, "morph", info, { Rotation = 45, Position = UDim2.new(0.5, 0, 0.5, 0),
            Size = UDim2.fromOffset(17, 2), BackgroundColor3 = C.AccentH })
        Motion.to(icon2, "morph", info, { Rotation = -45, Position = UDim2.new(0.5, 0, 0.5, 0),
            Size = UDim2.fromOffset(17, 2), BackgroundColor3 = C.AccentH })
    else
        Motion.to(icon1, "morph", info, { Rotation = 0, Position = UDim2.new(0.5, 0, 0.5, -4),
            Size = UDim2.fromOffset(18, 2), BackgroundColor3 = C.TxtMut })
        Motion.to(icon2, "morph", info, { Rotation = 0, Position = UDim2.new(0.5, 0, 0.5, 4),
...
local function startIdle()
    ...
    idleFloat = TweenService:Create(minBtn,
        TweenInfo.new(2.4, ES.Sine, ED.InOut, -1, true), { Rotation = 2.5 })
    idleFloat:Play()
end
...
local function showPanelAnimated()
    panel.Visible = true; menuVisible = true
    setBackdrop(true); morphIcon(true)
    ...
    if firstShow then
        ...
        task.delay(MOTION.reduce and 0.25 or 0.6, function()
            if firstLoad then typeIntro() end end)
    else
        ...
        task.delay(0.15, function()
            if menuVisible then startGlow(activeTab) end end)
    end
end
...
local function hidePanelAnimated()
    ...
    task.delay(0.22 * MOTION.speed, function()
        if not menuVisible then panel.Visible = false end end)
end
...
minBtn.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1
    or input.UserInputType == Enum.UserInputType.Touch then
        minDragging = true; minMoved = false; minDragStart = input.Position
        minStartPos = fabPos; minVel = Vector2.zero
        lastMinPos = Vector2.new(input.Position.X, input.Position.Y)
        lastMinT = os.clock(); stopIdle()
...
UserInputService.InputEnded:Connect(function(input)
    ...
    if dragging then
        ...
        Motion.to(pShadow, "drag", E.smooth(), { ImageTransparency = menuVisible and 0.5 or 0.9 })
    end
    if minDragging then
        minDragging = false
        ...
        if not minMoved then
            ...
            local vp = vpSize(); local pad = FAB/2 + 14
...
UserInputService.InputChanged:Connect(function(input)
    ...
    if minDragging and minDragStart then
        ...
        local now = os.clock()
        local dt2 = math.max(now - lastMinT, 1/240)
        local cur = Vector2.new(input.Position.X, input.Position.Y)
        minVel = (cur - lastMinPos) / dt2; lastMinPos, lastMinT = cur, now
        local vp = vpSize(); local pad = FAB/2 + 4
        fabPos = Vector2.new(
...
function typeIntro()
    typing = true
    frontLabel.Text = ""; frontLabel.TextSize = 18
    frontLabel.TextTransparency = 0; frontLabel.Position = UDim2.fromScale(0.5, 0.5)
...
local function tweenBar(target, dur)
    Motion.to(barFill, "size", ti(dur, ES.Quint, ED.Out), { Size = UDim2.new(target, 0, 1, 0) })
...
task.spawn(function()
    ...
    local ok, err = pcall(function()
        ...
        Motion.stagger({ wTitle, wCredit, barBack, wPct }, 0.08, 0.12, function(obj)
            if obj == barBack then Motion.to(obj, "fade", E.smooth(), { BackgroundTransparency = 0 })
            else Motion.to(obj, "fade", E.smooth(), { TextTransparency = 0 }) end
            if obj.Parent and obj:IsA("TextLabel") then
                obj.Position = obj.Position + UDim2.fromOffset(0, 6)
                Motion.to(obj, "pos", E.smooth(), { Position = obj.Position - UDim2.fromOffset(0, 6) })
            end
        end)
...
local UI = {
    ...
    header = header, Motion = Motion, C = C, E = E, ES = ES, ED = ED, ti = ti,
    new = new, corner = corner, dropShadow = dropShadow, borderStroke = borderStroke,
    addScale = addScale, TABS = TABS, TAB_NAMES = TAB_NAMES, navBtns = navBtns,
    navScales = navScales, styleNav = styleNav, startGlow = startGlow, stopGlow = stopGlow,
    crossfadeContent = crossfadeContent, frontLabel = frontLabel, backLabel = backLabel,
    toggleMenu = toggleMenu, small = small, PW = PW, PH = PH, NW = NW, NB = NB,
    HH = HH, NF = NF, vpSize = vpSize, LocalPlayer = LocalPlayer,
}
...
--============================================================
local mto = Motion.to
local mkill = Motion.kill
local mspring = Motion.spring
local mpress = Motion.press
local mstagger = Motion.stagger
local TS = TweenService
local GuiSvc = GuiService
...
local function makeSwitch(parent, opts)
    ...
    local track = new("TextButton", {
        Name = "SwitchTrack", Text = "", AutoButtonColor = false,
        BackgroundColor3 = TRK_OFF, BorderSizePixel = 0,
        AnchorPoint = Vector2.new(1, 0.5), Position = UDim2.new(1, -pad, 0.5, 0),
        Size = UDim2.fromOffset(40, 22), ZIndex = z,
    }, parent)
    round(track, UDim.new(1, 0)); edge(track, TRK_EDGE, 1, 0.4)
    local tsc = scaleOf(track, 1)
    local knob = new("Frame", {
        Name = "Knob", BackgroundColor3 = KNB_OFF, BorderSizePixel = 0,
        Size = UDim2.fromOffset(16, 16), AnchorPoint = Vector2.new(0.5, 0.5),
        Position = UDim2.new(0, KNOB_L, 0.5, 0), ZIndex = z + 1,
    }, track)
    ...
    local function paint(animate)
        ...
        if animate then
            mto(knob, "knobX", cti(0.30, "Back", "Out"), { Position = UDim2.new(0, kx, 0.5, 0) })
            mto(knob, "knobCol", cti(0.25, "Quint", "Out"), { BackgroundColor3 = kcl })
            mto(track, "trackBg", cti(0.25, "Quint", "Out"), { BackgroundColor3 = bg })
        else
            mkill(knob, "knobX"); mkill(knob, "knobCol"); mkill(track, "trackBg")
            knob.Position = UDim2.new(0, kx, 0.5, 0)
            knob.BackgroundColor3 = kcl; track.BackgroundColor3 = bg
        end
    end
    function api.getState() return state end
    function api.setState(on, animate, silent)
        on = on and true or false; local changed = (on ~= state)
        state = on; paint(animate ~= false)
    ...
    track.MouseButton1Click:Connect(function()
        mpress(tsc, 0.12); api.setState(not state, true)
    end)
    if not TOUCH then
        track.MouseEnter:Connect(function()
            hovering = true
            mto(track, "trackBg", cmicro(), { BackgroundColor3 = lighten(state and TRK_ON or TRK_OFF, 0.08) })
        end)
        track.MouseLeave:Connect(function()
            hovering = false
            mto(track, "trackBg", cmicro(), { BackgroundColor3 = state and TRK_ON or TRK_OFF })
        end)
    end
...
local function createToggle(parent, name, defaultState, onToggle, textSize)
    local row = new("Frame", {
        Name = "ToggleRow_"..tostring(name), Size = UDim2.new(1, 0, 0, 36),
        BackgroundTransparency = 1,
    }, parent)
    new("TextLabel", {
        Name = "Label", BackgroundTransparency = 1, Text = tostring(name),
        TextColor3 = C.TxtPri, TextXAlignment = Enum.TextXAlignment.Left,
        Font = FONT, TextSize = textSize or 13,
        Position = UDim2.new(0, 14, 0, 0), Size = UDim2.new(1, -80, 1, 0), ZIndex = 2,
    }, row)
    local sw = makeSwitch(row, { default = defaultState, onToggle = onToggle, rightPad = 14, zIndex = 2 })
    return { frame = row, switch = sw,
...
local function createAccordion(parent, name, hasToggle, defaultExpanded, textSize)
    ...
    local arrow = new("TextButton", {
        Name = "Arrow", Text = "▼", AutoButtonColor = false,
        BackgroundTransparency = 1, TextColor3 = C.TxtMut,
    ...
    function acc.setExpanded(on, animate)
        ...
        if animate then mto(arrow, "arrowRot", cti(0.28, "Back", "Out"), { Rotation = on and 0 or -90 })
        else mkill(arrow, "arrowRot"); arrow.Rotation = on and 0 or -90 end
        apply(animate)
    ...
    if not TOUCH then
        arrow.MouseEnter:Connect(function()
            mto(title, "hdrTxt", cmicro(), { TextColor3 = C.AccentH })
            mto(arrow, "hdrArw", cmicro(), { TextColor3 = C.TxtPri })
        end)
        arrow.MouseLeave:Connect(function()
            mto(title, "hdrTxt", cmicro(), { TextColor3 = C.TxtPri })
            mto(arrow, "hdrArw", cmicro(), { TextColor3 = C.TxtMut })
        end)
    end
    apply(false)
    return acc
...
local function openPicker(cfg)
    ...
    local backdrop = new("Frame", {
        Name = "ColorPickerOverlay", BackgroundColor3 = Color3.fromRGB(0, 0, 0),
        BackgroundTransparency = 1, BorderSizePixel = 0,
        Size = UDim2.fromScale(1, 1), Active = true, ZIndex = 100,
    }, gui)
    local dismiss = new("TextButton", {
        Name = "Dismiss", Text = "", AutoButtonColor = false,
        BackgroundTransparency = 1, Size = UDim2.fromScale(1, 1), ZIndex = 100,
    }, backdrop)
    local pnl = new("Frame", {
        Name = "PickerPanel", BackgroundColor3 = C.Panel, BorderSizePixel = 0,
        AnchorPoint = Vector2.new(0.5, 0.5), Position = UDim2.fromScale(0.5, 0.5),
        Size = UDim2.fromOffset(pw, ph), Active = true, ZIndex = 101,
    }, backdrop)
    round(pnl, 8); edge(pnl, C.StrkClr, 1, 0.15)
    dropShadow(pnl, 30, 0.55)
    local psc = scaleOf(pnl, 0.85)
    local sv = new("Frame", {
        Name = "SV", BackgroundColor3 = Color3.fromHSV(h, 1, 1),
        BorderSizePixel = 0, Position = UDim2.new(0, 12, 0, 12),
        Size = UDim2.new(1, -24, 0, svH), ClipsDescendants = true, ZIndex = 102,
    }, pnl)
    round(sv, 6)
    local white = new("Frame", {
        BackgroundColor3 = Color3.new(1, 1, 1), BorderSizePixel = 0,
        Size = UDim2.fromScale(1, 1), ZIndex = 103,
    }, sv)
    round(white, 6)
    new("UIGradient", { Transparency = NumberSequence.new({
    ...
    local black = new("Frame", {
        ...
        Size = UDim2.fromScale(1, 1), ZIndex = 104,
    }, sv)
    round(black, 6)
    ...
    local cursor = new("Frame", {
        BackgroundTransparency = 1, Size = UDim2.fromOffset(12, 12),
        AnchorPoint = Vector2.new(0.5, 0.5), Position = UDim2.fromScale(s, 1 - v),
        ZIndex = 105,
    }, sv)
    round(cursor, UDim.new(1, 0)); edge(cursor, Color3.new(1, 1, 1), 2, 0.05)
    ...
    local hue = new("Frame", {
        Name = "Hue", BackgroundColor3 = Color3.new(1, 1, 1), BorderSizePixel = 0,
        Position = UDim2.new(0, 12, 0, yHue), Size = UDim2.new(1, -24, 0, 16), ZIndex = 102,
    }, pnl)
    round(hue, 4)
    ...
    local hueInd = new("Frame", {
        ...
        Size = UDim2.fromOffset(6, 22), AnchorPoint = Vector2.new(0.5, 0.5),
        Position = UDim2.fromScale(h, 0.5), ZIndex = 104,
    }, hue)
    round(hueInd, 3); edge(hueInd, Color3.fromRGB(20, 20, 26), 1, 0.3)
    local hueHit = new("TextButton", {
        Text = "", AutoButtonColor = false, BackgroundTransparency = 1,
        Size = UDim2.new(1, 0, 1, 12), Position = UDim2.new(0, 0, 0, -6), ZIndex = 105,
    }, hue)
    local preview = new("Frame", {
        BackgroundColor3 = cfg.color, BorderSizePixel = 0,
        Position = UDim2.new(0, 12, 0, yPrev), Size = UDim2.fromOffset(28, 28), ZIndex = 102,
    }, pnl)
    round(preview, 6); edge(preview, SQ_EDGE, 1, 0)
    local rgbText = new("TextLabel", {
        BackgroundTransparency = 1, Font = FONT, TextSize = 12,
        TextColor3 = C.TxtMut, TextXAlignment = Enum.TextXAlignment.Left,
        Position = UDim2.new(0, 48, 0, yPrev), Size = UDim2.new(1, -60, 0, 28),
        Text = "", ZIndex = 102,
    }, pnl)
    local enRow = new("Frame", {
        BackgroundTransparency = 1, Position = UDim2.new(0, 12, 0, yEn),
        Size = UDim2.new(1, -24, 0, 30), ZIndex = 102,
    }, pnl)
    new("TextLabel", {
        BackgroundTransparency = 1, Font = FONT, TextSize = 12,
        TextColor3 = C.TxtPri, TextXAlignment = Enum.TextXAlignment.Left,
    ...
    local function push(smooth)
        ...
        if smooth then
            mto(cursor, "svCur", cti(0.06, "Quad", "Out"), { Position = UDim2.fromScale(s, 1 - v) })
            mto(hueInd, "hueInd", cti(0.08, "Quad", "Out"), { Position = UDim2.fromScale(h, 0.5) })
        else
            mkill(cursor, "svCur"); mkill(hueInd, "hueInd")
            cursor.Position = UDim2.fromScale(s, 1 - v); hueInd.Position = UDim2.fromScale(h, 0.5)
        end
    ...
    function api.close()
        ...
        table.clear(conns)
        mto(backdrop, "fade", cti(0.18, "Quint", "In"), { BackgroundTransparency = 1 })
        mto(psc, "pop", cti(0.18, "Quint", "In"), { Scale = 0.9 })
        task.delay(0.2, function() if backdrop and backdrop.Parent then backdrop:Destroy() end end)
        if cfg.onClose then cfg.onClose() end
    end
    dismiss.MouseButton1Click:Connect(api.close)
    push(false)
    mto(backdrop, "fade", cti(0.20, "Quad", "Out"), { BackgroundTransparency = 0.5 })
    mspring(psc, 1)
    activePicker = api
    return api
end
...
local function createColorSetting(parent, name, defaultColor, defaultEnabled, onColorChange, textSize)
    ...
    local row = new("Frame", {
        Name = "ColorRow_"..tostring(name), Size = UDim2.new(1, 0, 0, 36),
        BackgroundTransparency = 1,
    }, parent)
    new("TextLabel", {
        Name = "Label", BackgroundTransparency = 1, Text = tostring(name),
        TextColor3 = C.TxtPri, TextXAlignment = Enum.TextXAlignment.Left,
        Font = FONT, TextSize = textSize or 12,
        Position = UDim2.new(0, 14, 0, 0), Size = UDim2.new(1, -104, 1, 0), ZIndex = 2,
    }, row)
    local square = new("TextButton", {
        Name = "ColorSquare", Text = "", AutoButtonColor = false,
        BackgroundColor3 = color, BorderSizePixel = 0,
        Size = UDim2.fromOffset(20, 20), AnchorPoint = Vector2.new(1, 0.5),
        Position = UDim2.new(1, -64, 0.5, 0), ZIndex = 3,
    }, row)
    round(square, 4); edge(square, SQ_EDGE, 1, 0)
    local ssc = scaleOf(square, 1)
    local cross = new("TextLabel", {
        Name = "Off", BackgroundTransparency = 1, Text = "✕",
        Font = FONT, TextSize = 12, TextColor3 = C.TxtHint,
        Size = UDim2.fromScale(1, 1), Visible = false, ZIndex = 4,
    }, square)
    ...
    local function paintSquare(animate)
        ...
        if animate then
            mto(square, "sqFade", cmicro(), { BackgroundTransparency = target, BackgroundColor3 = color })
        else
            mkill(square, "sqFade")
            square.BackgroundTransparency = target; square.BackgroundColor3 = color
        end
    end
...
local function createSlider(parent, name, minV, maxV, default, suffix, onValueChange)
    ...
    local row = new("Frame", {
        Name = "SliderRow_"..tostring(name), Size = UDim2.new(1, 0, 0, 44),
        BackgroundTransparency = 1,
    }, parent)
    new("TextLabel", {
        Name = "Label", BackgroundTransparency = 1, Text = tostring(name),
        ...
        Position = UDim2.new(0, 14, 0, 0), Size = UDim2.new(0.5, 0, 0, 20), ZIndex = 2,
    }, row)
    local valueTxt = new("TextLabel", {
        Name = "Value", BackgroundTransparency = 1, Text = "",
        TextColor3 = C.TxtMut, TextXAlignment = Enum.TextXAlignment.Right,
        Font = FONT, TextSize = 12, AnchorPoint = Vector2.new(1, 0),
        Position = UDim2.new(1, -14, 0, 0), Size = UDim2.new(0, 60, 0, 20), ZIndex = 2,
    }, row)
    local track = new("Frame", {
        Name = "Track", BackgroundColor3 = TRACK_BG, BorderSizePixel = 0,
        Position = UDim2.new(0, 14, 0, 26), Size = UDim2.new(1, -28, 0, 6), ZIndex = 2,
    }, row)
    round(track, 3)
    local fill = new("Frame", {
        Name = "Fill", BackgroundColor3 = C.Accent, BorderSizePixel = 0,
        Size = UDim2.new(0, 0, 1, 0), ZIndex = 3,
    }, track)
    round(fill, 3)
    new("UIGradient", { Color = ColorSequence.new(C.BarA, C.BarB) }, fill)
    local handle = new("Frame", {
        Name = "Handle", BackgroundColor3 = C.TxtPri, BorderSizePixel = 0,
        Size = UDim2.fromOffset(14, 14), AnchorPoint = Vector2.new(0.5, 0.5),
        Position = UDim2.new(0, 0, 0.5, 0), ZIndex = 4,
    }, track)
    round(handle, UDim.new(1, 0))
    local hsc = scaleOf(handle, 1)
    local hit = new("TextButton", {
        Name = "Hit", Text = "", AutoButtonColor = false,
        BackgroundTransparency = 1,
    ...
    local function paint(animate)
        local f = frac()
        if animate then
            mto(fill, "sldFill", cti(0.25, "Quint", "Out"), { Size = UDim2.new(f, 0, 1, 0) })
            mto(handle, "sldPos", cti(0.25, "Quint", "Out"), { Position = UDim2.new(f, 0, 0.5, 0) })
        else
            mkill(fill, "sldFill"); mkill(handle, "sldPos")
            fill.Size = UDim2.new(f, 0, 1, 0); handle.Position = UDim2.new(f, 0, 0.5, 0)
        end
    ...
    function api.setValue(newValue, animate, silent)
        ...
        local changed = q ~= value; value = q; paint(animate == true)
...
local function createESPSection(parent, name, defaultColor, hasHealthStatus, hasProgress, settings_ref)
    ...
    if acc.toggle then
        acc.toggle.onToggle = function(on) settings_ref.enabled = on end
    end
    ...
    if hasProgress then
        createToggle(acc.content, "Progress", false, function(on) settings_ref.progress = on end, 12)
    end
    createColorSetting(acc.content, "Outline Color", defaultColor, true,
        function(color, enabled) settings_ref.outline.color = color; settings_ref.outline.enabled = enabled end, 12)
    createColorSetting(acc.content, "Fill Color", defaultColor, false,
        function(color, enabled) settings_ref.fill.color = color; settings_ref.fill.enabled = enabled end, 12)
    return acc
end
...
local function buildVisualTab(parent)
    local scroll = new("ScrollingFrame", {
        Name = "VisualContent", BackgroundTransparency = 1, BorderSizePixel = 0,
...
print("[gw.cc] All-in-One loaded successfully!")
